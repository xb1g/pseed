...
import { getCommunityBySlug, listCommunityPosts, joinCommunity, leaveCommunity, createPost } from '@/lib/api/community';
...
export default function CommunityPage() {
  ...
  const loadPosts = async (communityId: string, reset = false) => {
    ...
    try {
      const { data, count } = await listCommunityPosts(communityId, currentPage);
  ...
  const isAdmin = community.member_role === 'admin' || community.member_role === 'owner';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                {posts.length > 0 ? (
                  posts.map((post) => (
                    <PostCard 
                      ...
                      onComment={() => {
                        // In a real app, you would scroll to comments or open a modal
                      ...
                      onDelete={async () => {
                        // In a real app, you would call the delete post API
            ...
            <TabsContent value="members">
              <div className="bg-card rounded-lg border p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {/* In a real app, you would fetch and map through community members */}
                  {[1, 2, 3, 4, 5, 6].map((i) => (
                  ...
                  <div className="mt-6 pt-6 border-t">
                    ...
                    <div className="grid gap-3">
                      ...
                      <div className="flex items-center">
                        ...
                        <span>{community.is_public ? 'Public' : 'Private'} community</span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="space-y-6">
              ...
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{Math.floor(Math.random() * 1000)} members</p>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-card rounded-lg border p-4 text-sm text-muted-foreground">
            <p>© {new Date().getFullYear()} {community.name}. All rights reserved.</p>
          </div>
        </div>
      </div>
    </div>
