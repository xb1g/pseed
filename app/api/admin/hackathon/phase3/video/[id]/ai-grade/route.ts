...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: row } = await serviceClient.from("hackathon_phase3_video_submissions").select("*").eq("id", id).single();
  if (!row) return NextResponse.json({ error: "Video not found" }, { status: 404 });

  const prompt = buildPhase3GradingPrompt({ entityType: "video", entityData: row as Record<string, unknown> });
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      await serviceClient
        ...
        .update({ ai_scrutinizer_output: { total: score }, updated_at: now })
        .eq("id", id);

      recalculateAndUpsertTeamScore(serviceClient, (row as any).team_id).catch(() => {});

      send({ type: "done", score, feedback: outcome.draft.feedback, reasoning: outcome.draft.reasoning, consensus: outcome.draft.consensus });
      try { controller.close(); } catch {}
    },
    cancel() {},
  });
