...
const CONTENT = {
  ...
  th: {
    ...
    greeting: (name: string) => `สวัสดี ${name} เรามาหาสิ่งที่ทำให้คุณรู้สึกอยากไปต่อกัน`,
...
export function WelcomePhase({ data, oauthName, advance }: WelcomePhaseProps) {
  ...
  return (
    <section className="w-full max-w-3xl">
      <div className="ei-card relative overflow-hidden rounded-[28px] border border-white/10 px-6 py-8 sm:px-10 sm:py-10">
        ...
        <div className="relative flex flex-col gap-8">
          <div className="flex items-start justify-between gap-4">
            <div className="max-w-xl space-y-3">
              <span className="inline-flex rounded-full border border-orange-300/25 bg-orange-300/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.24em] text-orange-200">
                {content.badge}
              </span>
              <div className="space-y-2">
                <h1 className="text-3xl font-semibold tracking-tight text-white sm:text-4xl">
                  {isOauthUser ? content.greeting(trimmedName || oauthName || '') : content.nameLabel}
                </h1>
          ...
          {!isOauthUser ? (
            <label className="flex flex-col gap-2">
              <span className="text-sm font-medium text-white/75">{content.nameLabel}</span>
              <input
                autoFocus
                value={name}
          ...
          <div className="space-y-3">
            <p className="text-sm font-medium text-white/72">{content.helper}</p>
            <div className="grid gap-3 md:grid-cols-2">
              {([
                ...
                },
                {
                  ...
                  accent: 'from-violet-300/18 via-fuchsia-400/12 to-transparent',
                },
              ] as const).map((option) => {
                  ...
                  >
                    ...
                    <div className="relative space-y-3">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <p className="text-sm leading-6 text-white/65">{option.description}</p>
                      </div>
                    </div>
