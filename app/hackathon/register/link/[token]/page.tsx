...
export default function LinkRegisterPage() {
  ...
  useEffect(() => {
    fetch(`/api/hackathon/register/link/${token}`)
      .then((res) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const apiPromise = fetch(`/api/hackathon/register/link/${token}`, {
      method: "POST",
    ...
    >
      <style jsx>{`
        ...
        @keyframes shorePulse {
          0%, 100% { opacity: 0.55; box-shadow: 0 0 18px 4px rgba(101,171,252,0.35), 0 0 40px 10px rgba(145,196,227,0.15); }
          50% { opacity: 1; box-shadow: 0 0 28px 7px rgba(101,171,252,0.65), 0 0 60px 18px rgba(145,196,227,0.3); }
        }
        ...
        @keyframes formBreath {
          0%, 100% { box-shadow: 0 0 50px rgba(70,130,200,0.06), 0 30px 80px rgba(0,0,0,0.6), inset 0 0 40px rgba(70,130,200,0.02); }
          50% { box-shadow: 0 0 70px rgba(70,130,200,0.11), 0 30px 80px rgba(0,0,0,0.6), inset 0 0 60px rgba(70,130,200,0.04); }
        }
        @keyframes titleGlow {
      ...
      <div className="relative z-[30] w-full max-w-lg px-6 py-10">
        ...
        {validating ? (
          <div className="text-center" style={{ color: "#3A5A70" }}>
            <p className="font-[family-name:var(--font-mitr)]">กำลังตรวจสอบลิงก์...</p>
          </div>
        ) : invalid ? (
          <div
            className="rounded-2xl p-8 form-card text-center"
            style={{ background: "rgba(1, 5, 12, 0.9)", border: "1px solid rgba(255,80,80,0.2)", backdropFilter: "blur(16px)" }}
          >
            <p className="text-lg font-medium mb-2 font-[family-name:var(--font-mitr)]" style={{ color: "#FF8080" }}>
              ลิงก์ไม่ถูกต้อง
            </p>
            <p className="text-sm font-[family-name:var(--font-mitr)]" style={{ color: "#3A5A70" }}>
              ลิงก์นี้ถูกใช้ไปแล้วหรือไม่ถูกต้อง กรุณาติดต่อผู้จัดงาน
            </p>
          </div>
        ) : (
          <div
            className="rounded-2xl p-8 form-card"
            style={{ background: "rgba(1, 5, 12, 0.9)", border: "1px solid rgba(101,171,252,0.2)", backdropFilter: "blur(16px)" }}
          >
            <h1
