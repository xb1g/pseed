...
function parseEmbedding(raw: number[] | string): number[] {
  ...
  // pgvector returns "[0.1,0.2,...]" as a string when cast to json.
  const trimmed = raw.trim().replace(/^\[|\]$/g, "");
  return trimmed.split(",").map((v) => Number(v));
}

/**
 * Recluster all submissions for an activity. Writes one
 * `activity_submission_clusterings` row (flipping previous `is_latest`),
 * one row per cluster in `activity_submission_clusters`, and one assignment
 * per embedding in `submission_cluster_assignments`.
 */
export async function reclusterActivity(opts: RecllusterOptions): Promise<ReclusterResult> {
  ...
  if (sampleCount === 0) {
    throw new Error("No submission embeddings found for this activity");
  }
  ...
  // Flip any previous latest clustering for this activity to is_latest=false
  // before inserting the new one. Wrap in try/catch so partial failure surfaces.
  ...
  // Postgrest caps payloads; chunk for safety.
  const CHUNK = 500;
