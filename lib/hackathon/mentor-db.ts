...
export async function findMentorById(id: string): Promise<MentorProfile | null> {
  const { data } = await getClient()
    .from("mentor_profiles")
    .select(
      "id, user_id, full_name, email, profession, institution, bio, photo_url, line_user_id, instagram_url, linkedin_url, website_url, max_hours_per_week, session_type, is_approved, is_accepting_bookings, created_at, updated_at"
    )
    .eq("id", id)
    .single();
...
export async function getMentorBySessionToken(token: string): Promise<MentorProfile | null> {
  ...
  const { data } = await getClient()
    ...
    .select(
      "expires_at, mentor_profiles(id, user_id, full_name, email, profession, institution, bio, photo_url, line_user_id, instagram_url, linkedin_url, website_url, max_hours_per_week, session_type, is_approved, is_accepting_bookings, created_at, updated_at)"
    )
    .eq("token", token)
    .gt("expires_at", now)
    .single();
...
export async function getMentorByLineUserId(lineUserId: string): Promise<MentorProfile | null> {
  const { data } = await getClient()
    .from("mentor_profiles")
    .select("id, user_id, full_name, email, profession, institution, bio, photo_url, line_user_id, instagram_url, linkedin_url, website_url, max_hours_per_week, session_type, is_approved, is_accepting_bookings, created_at, updated_at")
