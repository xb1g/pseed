...
import {
  JourneyActionBar,
  JourneyStats,
} from "@/components/journey/JourneyActionBar";
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs text-slate-400">{university.name}</span>
            </div>
          </div>
        </div>
...
export function UniversityExampleJourneyCanvas({
  ...
  const [exampleMapData, setExampleMapData] =
    React.useState<ExampleMapMetadata>({
  ...
  const [existingMapIdState, setExistingMapIdState] = React.useState<
    string | null
  >(existingMapId || null);
  ...
  const [savedNorthStarPosition, setSavedNorthStarPosition] = React.useState<{
    x: number;
    y: number;
  } | null>(null);
  ...
  React.useEffect(() => {
    if (initialExampleData) {
      try {
        ...
        if (
          initialExampleData.milestones &&
          Array.isArray(initialExampleData.milestones)
        ) {
          const loadedProjects = initialExampleData.milestones.map(
        ...
        if (initialExampleData.metadata?.north_star) {
          ...
          if (
            northStar.position_x !== undefined &&
            northStar.position_y !== undefined
          ) {
      ...
      try {
        ...
        if (existingMaps.length > 0) {
          ...
          if (latestMap.example_data?.milestones) {
            const loadedProjects = latestMap.example_data.milestones.map(
              (milestone: any, index: number) => ({
                id: `milestone-${index + 1}`,
                user_id: user.id,
                title: milestone.title,
                ...
                is_main_quest: false,
                position_x:
                  milestone.position_x !== undefined ? milestone.position_x : 0,
                position_y:
                  milestone.position_y !== undefined
                    ? milestone.position_y
                    : -100 + index * 200,
                status: "not_started",
                color_theme: null,
                start_date: null,
          ...
          if (latestMap.example_data?.metadata?.north_star) {
            ...
            if (
              northStarData.position_x !== undefined &&
              northStarData.position_y !== undefined
            ) {
              setSavedNorthStarPosition({
                x: northStarData.position_x,
                y: northStarData.position_y,
              });
              console.log("Loaded north star position:", {
                x: northStarData.position_x,
                y: northStarData.position_y,
              });
            }
          ...
          project.id === projectId
            ? {
                ...
                metadata: {
                  ...
                  importance:
                    updateData.importance || project.metadata?.importance,
                  target_timeframe:
                    updateData.target_timeframe ||
                    project.metadata?.target_timeframe,
                },
                updated_at: "2024-01-01T00:00:00.000Z", // Static for SSR consistency
              }
  ...
  const handlePathCreate = useCallback(
    async (fromId: string, toId: string, pathType: string) => {
  ...
  const journeyStats = useMemo(() => {
    ...
    const validProjectsArray = projectsArray.filter(
      (p) => p && typeof p === "object",
    );
    ...
    try {
      ...
      const milestonesData = projects.map((project) => {
        ...
        const position = currentNode
          ...
          : { x: project.position_x || 0, y: project.position_y || 0 };
      ...
      const northStarNode = nodes.find(
        (node) => node.id === universityNorthStar.id,
      );
      const northStarPosition = northStarNode
        ? northStarNode.position
        : { x: 0, y: -300 };
      ...
      toast.success(
        existingMapIdState
          ? "Example journey map updated successfully"
          : "Example journey map created successfully",
      );
                  ...
                  {isLoading ? (
                    <div className="flex items-center justify-center h-full">
                      <div className="text-center">
                        ...
                        <p className="text-slate-400">
                          Loading university example map...
                        </p>
                      </div>
                    </div>
                  ) : (
                    <ReactFlow
                      ...
                      onConnect={() => {
                        ...
                        console.log(
                          "Path creation disabled for university examples",
                        );
                      }}
                      nodeTypes={nodeTypes}
                      edgeTypes={edgeTypes}
                    ...
                    <Card className="bg-slate-800 border-slate-600">
                      ...
                      <CardContent className="space-y-4">
                        <p className="text-slate-300 text-sm">
                          Create milestones that guide students toward admission
                          to this university.
                        </p>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-800 rounded-lg p-4 space-y-2 text-sm">
              ...
              <div>
                <strong>Milestones:</strong> {projects.length}
              </div>
              <div>
                <strong>North Star:</strong> Get into {university.name}
              </div>
            </div>
          </div>
