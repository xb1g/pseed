...
export function TaskCard({
  ...
  getDifficultyColor,
  getStatusIcon,
}: DraggableTaskProps & { isOverlay?: boolean }) {
  return (
    <Card
        ...
        <div className="flex items-center justify-between pt-2 mt-auto border-t border-border/50">
          <div className="flex gap-2 items-center">
            ...
            {task.feedback_count && task.feedback_count > 0 ? (
              <Badge
                variant="outline"
                className="text-[10px] h-5 px-1.5 flex gap-1 items-center border-orange-200 text-orange-700 bg-orange-50"
              >
                <MessageSquare className="h-3 w-3" /> {task.feedback_count}
              </Badge>
            ) : null}
          </div>
