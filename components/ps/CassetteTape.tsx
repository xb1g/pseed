...
export function CassetteTape({ project }: CassetteTapeProps) {
    ...
    return (
        <Link
            ...
            className="group block relative w-full aspect-[1.6/1] max-w-[320px] mx-auto transition-transform hover:scale-105 hover:-rotate-1 duration-300"
        >
            {/* Cassette Body */}
            <div
                className={`absolute inset-0 rounded-xl shadow-xl overflow-hidden border-2 border-neutral-700/50 ${!isCustomTheme ? 'bg-neutral-800' : ''}`}
                style={tapeStyle}
            >
                {/* Screw holes */}
                ...
                {/* Top Label Area */}
                <div
                    className={`absolute top-[10%] left-[5%] right-[5%] h-[60%] rounded-md shadow-sm p-3 flex flex-col ${!isCustomTheme ? defaultLabelColor : ''}`}
                    style={labelStyle}
                >
                    ...
                    <div
                        ...
                        style={isCustomTheme ? { borderColor: `${theme.labelStyle?.color}40` } : {}}
                    >
                        {project.name}
                    </div>
                    ...
                    <div className="flex-1 space-y-1 relative">
                        <div className="flex justify-between items-start">
                            <p className={`text-[10px] line-clamp-2 font-mono leading-tight flex-1 mr-2 ${!isCustomTheme ? 'text-neutral-800/80' : 'opacity-80'}`}>
                                {project.goal || project.description || "No tracks recorded yet..."}
                            </p>
                            {project.spotify_artist_name && (
                                <div className={`text-[9px] font-bold border rounded px-1 max-w-[40%] truncate ${!isCustomTheme ? 'text-neutral-800 border-neutral-800/30' : ''}`}
                                    style={isCustomTheme ? { borderColor: `${theme.labelStyle?.color}50` } : {}}
                                >
                                    {project.spotify_artist_name}
                                </div>
                            )}
                        ...
                        <div className={`absolute bottom-1 right-1 border-2 rounded px-1 text-[8px] font-bold ${!isCustomTheme ? 'border-neutral-800/80 text-neutral-800' : ''}`}
                            style={isCustomTheme ? { borderColor: `${theme.labelStyle?.color}CC` } : {}}
                        >
                            SIDE A
                        </div>

                        {project.spotify_track_name && (
                            <div className={`absolute bottom-1 left-0 text-[9px] italic truncate max-w-[70%] ${!isCustomTheme ? 'text-neutral-800/70' : 'opacity-70'}`}>
                ...
                <div className="absolute bottom-[10%] left-[15%] right-[15%] h-[25%] bg-neutral-900/40 rounded-full flex items-center justify-center gap-4 px-2 border border-neutral-700/50 relative overflow-hidden backdrop-blur-sm">
                    ...
                    {/* Left Reel */}
                    <div className="w-8 h-8 rounded-full bg-white/90 border-2 border-neutral-600 relative animate-spin-slow group-hover:animate-spin">
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/20 rounded-xl pointer-events-none">
                <div className="bg-primary text-primary-foreground p-3 rounded-full shadow-lg transform scale-90 group-hover:scale-110 transition-transform">
