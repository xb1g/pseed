...
export function StudentProgressTable({
  ...
  const getStudentName = (student: Student) => {
    console.log(
      "🎭 [DEBUG] Getting student name for student ID:",
      student.user_id,
    );
    console.log(
      "🔍 [DEBUG] Full student object:",
      JSON.stringify(student, null, 2),
    );
    console.log(
      "👤 [DEBUG] User object:",
      JSON.stringify(student.user, null, 2),
    );
  ...
  const getOverallProgress = (student: Student) => {
    ...
    const totalProgress = student.map_progress.reduce((sum, progress) => {
  ...
  const studentProgressMaps = useMemo(() => {
    ...
    students.forEach((student) => {
      if (student.map_progress) {
        mapsByStudent.set(
          student.id,
          new Map(student.map_progress.map((p) => [p.map_id, p])),
        );
      } else {
  ...
  const handleRemoveStudent = async () => {
    ...
    try {
      console.log(
        "🔵 [RemoveStudent] Deleting classroom_memberships record...",
      );
      ...
      toast.success(
        `${getStudentName(studentToRemove)} has been removed from the classroom`,
      );
              ...
              {students.length === 0 ? (
                <TableRow>
                  <TableCell
                    colSpan={
                      5 +
                      (maps.length > 3 ? 1 : Math.min(maps.length, 3)) +
                      (canManage ? 1 : 0)
                    }
                ...
                students.map((student) => (
                  <TableRow key={student.id}>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          {(student.user?.email === "No email available" ||
                            student.user?.full_name === null) && (
                            <button
                              onClick={async () => {
                                console.log(
                                  "🔍 [DEBUG] Checking user data for:",
                                  student.user_id,
                                );
                                try {
                                  const response = await fetch(
                                    `/api/debug/user/${student.user_id}`,
                                  );
                                  const data = await response.json();
                                  console.log(
                                    "🎯 [DEBUG] User diagnostic data:",
                                    data,
                                  );
                                } catch (error) {
                                  console.error(
                                    "💥 [DEBUG] Error fetching user debug data:",
                                    error,
                                  );
                                }
                              }}
                    ...
                    {maps.slice(0, 3).map((map) => {
                      ...
                      const progress = progressMap?.get(map.map_id);
                      return (
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem
                              ...
                              onClick={() => {
                                console.log(
                                  "🔵 [RemoveStudent] Remove button clicked for:",
                                  student.user_id,
                                );
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              from this classroom? This will remove their access to all
