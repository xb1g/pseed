  ...
  useEffect(() => {
    const loadUserGroup = async () => {
      ...
      try {
        ...
        let group;
        if (mapData && isSeedMap(mapData)) {
          console.log("🌱 Seed map detected, using seed room members as group");
          group = await getSeedRoomAssessmentGroup(assessment.id, user.id);
        } else {
          console.log("📚 Classroom map, using assessment groups");
          group = await getUserAssessmentGroup(assessment.id, user.id);
        }
    ...
    loadUserGroup();
  }, [assessment.id, assessment.is_group_assessment, user, mapId, toast]);
