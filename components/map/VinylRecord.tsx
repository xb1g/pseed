...
export function VinylRecord({
  ...
  return (
    <div className={`absolute left-1/2 -translate-x-1/2 -top-16 w-72 h-72 transition-all duration-700 ${isAnimating ? 'group-hover:-translate-y-20' : ''}`} style={{ zIndex: 1 }}>
      <div className="relative w-full h-full">
        <div
          className={`vinyl-record w-full h-full rounded-full shadow-2xl border-4 relative overflow-hidden ${
            !hasImage ? 'bg-gradient-to-br from-gray-900 via-black to-gray-800 border-gray-700' : 'border-opacity-70'
          }`}
        ...
        >
          ...
          <div
            ...
            style={vinylColors?.grooveStyle ? vinylColors.grooveStyle : { borderColor: 'rgba(156, 163, 175, 0.5)' }}
          />
          <div
            className="absolute inset-8 rounded-full border opacity-25"
            style={vinylColors?.grooveStyle ? vinylColors.grooveStyle : { borderColor: 'rgba(156, 163, 175, 0.3)' }}
          />
          <div
            className="absolute inset-12 rounded-full border opacity-15"
            style={vinylColors?.grooveStyle ? vinylColors.grooveStyle : { borderColor: 'rgba(156, 163, 175, 0.2)' }}
          />
          <div
            className="absolute inset-16 rounded-full border opacity-10"
            style={vinylColors?.grooveStyle ? vinylColors.grooveStyle : { borderColor: 'rgba(156, 163, 175, 0.1)' }}
          />
          ...
          <div
            ...
            style={vinylColors?.labelStyle ? vinylColors.labelStyle : {
