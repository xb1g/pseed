-- Claim one subscription's delivery work in a single transaction. Frozen
-- retries are selected before new work and are always leased as a complete
-- cohort, so a provider idempotency key can never be reused with new content.
...
begin
  ...
  -- Lock exactly one eligible subscription. A frozen retry row sorts before
  -- every unfrozen row even when the new row has an earlier scheduled_at.
