-- Keep full-draft /plan sync from regressing a newer atomic Radar signal.
-- This is an additive replacement of the current private sync implementation;
-- the public invoker wrapper and its authenticated grants remain unchanged.
...
begin
  ...
  loop
    ...
    -- A full plan draft can have been opened before a Radar event was accepted.
    -- Only its possibility fields may overwrite the row when that signal is
    -- strictly newer; unrelated plan, event, question, and step edits continue.
    set state = case
          when excluded.last_interaction_at > public.my_path_possibilities.last_interaction_at
            then excluded.state
        ...
        saved_at = case
          when excluded.last_interaction_at > public.my_path_possibilities.last_interaction_at
            then coalesce(excluded.saved_at, public.my_path_possibilities.saved_at)
        ...
        removal_reason = case
          when excluded.last_interaction_at > public.my_path_possibilities.last_interaction_at
            then coalesce(excluded.removal_reason, public.my_path_possibilities.removal_reason)
