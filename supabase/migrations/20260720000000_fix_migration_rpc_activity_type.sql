-- =====================================================
-- FIX PATHLAB MIGRATION RPC FUNCTION
-- Removes deprecated activity_type, fixes node_content.metadata lookup,
-- fixes quiz_questions.created_at schema mismatch, and resolves duplicate display orders.
-- =====================================================
      ...
      IF day_record.node_ids IS NOT NULL THEN
        ...
        LOOP
          ...
          IF node_title IS NOT NULL THEN
            ...
            -- Reset content index counter for this node
            content_idx := 0;

            -- Migrate node_content to path_content (generating sequential display_order to avoid duplicates)
            ...
            LOOP
              INSERT INTO public.path_content (
                ...
                content_idx, -- Use clean sequential counter instead of potentially duplicated content_record.display_order
                '{}'::jsonb, -- Default empty JSONB metadata
            ...
            LOOP
              ...
              -- Migrate quiz questions if quiz type (omitting non-existent created_at column)
              IF assessment_record.assessment_type = 'quiz' THEN
                ...
                LOOP
                  INSERT INTO public.path_quiz_questions (
                    ...
                    now() -- Use current time for default creation
                  );
