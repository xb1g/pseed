...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_name='learning_maps' AND column_name='parent_classroom_id'
    ) THEN
        ALTER TABLE public.learning_maps
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name='learning_maps_classroom_exclusive_check'
        AND table_name='learning_maps'
    ) THEN
        ALTER TABLE public.learning_maps
...
CREATE INDEX IF NOT EXISTS learning_maps_map_type_idx ON public.learning_maps USING btree (map_type);
CREATE INDEX IF NOT EXISTS learning_maps_parent_classroom_idx ON public.learning_maps USING btree (parent_classroom_id) WHERE parent_classroom_id IS NOT NULL;

-- ========================================
-- 2. CREATE CLASSROOM MAP FEATURES TABLE
-- ========================================

-- Check if classroom_map_features table exists, if not create it
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='classroom_map_features') THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tablename = 'learning_maps' AND policyname = 'view_maps_policy'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'learning_maps' AND policyname = 'create_maps_policy'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'learning_maps' AND policyname = 'update_maps_policy'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'learning_maps' AND policyname = 'delete_maps_policy'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'classroom_map_features' AND policyname = 'view_classroom_map_features'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'classroom_map_features' AND policyname = 'manage_classroom_map_features'
    ) THEN
...
BEGIN
    ...
    RETURN result;
END;
$func1$ LANGUAGE plpgsql SECURITY DEFINER;
