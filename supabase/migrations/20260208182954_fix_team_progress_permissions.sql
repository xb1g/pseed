-- Fix permission issues with update_team_progress_from_individual function
-- The function needs SECURITY DEFINER to access team_memberships and related tables
...
BEGIN
  ...
  -- Exit if this user is not in a team for this map (e.g., solo PathLab context)
...
COMMENT ON FUNCTION "public"."update_team_progress_from_individual"() IS 'Trigger function to automatically sync team progress when individual node progress changes. Uses SECURITY DEFINER to access team_memberships and related tables with owner privileges. Exits early for solo contexts (e.g., PathLab).';
