-- =====================================================
-- FIX PATHLAB MIGRATION AMBIGUOUS COLUMN REFERENCES
-- Updates the migration functions to fix PostgreSQL errors
-- =====================================================

-- Update main migration function with qualified column names
...
-- Update migrate_single_path with qualified column names
...
BEGIN
  ...
  WHERE path_days.path_id = p_path_id;

  -- Run migration for this path only
  RETURN QUERY
  SELECT m.path_day_id, m.activities_created, m.content_items_migrated,
         m.assessments_migrated, m.quiz_questions_migrated, m.status, m.error_message
  FROM migrate_pathlab_nodes_to_activities() m
  WHERE m.path_day_id IN (
    SELECT pd.id FROM public.path_days pd WHERE pd.path_id = p_path_id
  );
END;
$$ LANGUAGE plpgsql;
