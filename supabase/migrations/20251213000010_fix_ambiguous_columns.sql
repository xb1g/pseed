...
-- The issue: multiple tables have user_id columns, need to fully qualify them
...
-- Function: Get leaderboard for a seed room (FIXED)
...
BEGIN
    ...
    user_points AS (
        ...
        WHERE snp.user_id IN (SELECT cu.user_id FROM completed_users cu)
    ...
    ranked_users AS (
        SELECT 
            up.uid,
            up.total_points,
            RANK() OVER (ORDER BY up.total_points DESC) as user_rank
        FROM user_points up
    )
    SELECT 
        ru.uid as user_id,
    ...
    JOIN profiles p ON p.id = ru.uid
    ORDER BY ru.user_rank ASC
    LIMIT p_limit;
END;
$$ LANGUAGE plpgsql STABLE;

-- Function: Get total points for a specific user in a seed room (FIXED)
...
-- Function: Calculate percentile for a user in a seed room (FIXED)
...
BEGIN
    ...
    completed_users AS (
        SELECT DISTINCT src.user_id as uid
        FROM seed_room_completions src
        WHERE src.room_id = p_room_id
    ),
    user_points AS (
        ...
        JOIN assessment_submissions asub ON asub.progress_id = snp.id
        LEFT JOIN submission_grades sg ON sg.submission_id = asub.id
        WHERE snp.user_id IN (SELECT cu.uid FROM completed_users cu)
        GROUP BY snp.user_id
    ),
    ranked_users AS (
        SELECT 
