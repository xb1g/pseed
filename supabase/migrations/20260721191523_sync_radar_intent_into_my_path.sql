-- Atomically append one authenticated Radar field intent into My Path.
-- This avoids read/merge/write races against the full journey draft sync.
...
begin
  ...
  insert into public.my_path_events (
    ...
    p_career_slug, jsonb_build_object('source', 'radar'), p_occurred_at
  )
  ...
  if v_current_interaction is not null and v_current_interaction > p_occurred_at then
  ...
  if p_event_type = 'radar_profile_opened' then
    ...
    if v_current_state = 'saved' or v_saved_count < 3 then
      insert into public.my_path_possibilities (
        ...
        v_path_id, v_user_id, p_career_slug, 'saved', p_occurred_at,
        p_occurred_at, p_occurred_at
      )
      on conflict (my_path_id, radar_slug) do update
      set state = 'saved',
    ...
    insert into public.my_path_possibilities (
      ...
      v_path_id, v_user_id, p_career_slug, 'removed', p_occurred_at,
      'radar_intent', p_occurred_at, p_occurred_at
    )
    on conflict (my_path_id, radar_slug) do update
    set state = 'removed',
...
create or replace function public.apply_my_path_radar_event(
  p_client_event_id text,
  p_event_type text,
  p_career_slug text,
  p_occurred_at timestamptz
...
set search_path = ''
as $$
  select private.apply_my_path_radar_event(
    p_client_event_id,
    p_event_type,
    p_career_slug,
    p_occurred_at
...
revoke all on function private.apply_my_path_radar_event(text, text, text, timestamptz)
  from public;
revoke all on function public.apply_my_path_radar_event(text, text, text, timestamptz)
  from public;
grant execute on function private.apply_my_path_radar_event(text, text, text, timestamptz)
  to authenticated;
grant execute on function public.apply_my_path_radar_event(text, text, text, timestamptz)
  to authenticated;
